...
def get_labeled_image(class_names, classes, masks, labeled_image, img):
    for i in range(masks.shape[0]):
        ...
        mask_resized = cv2.resize(np.array(masks[i].cpu()), (img.shape[1], img.shape[0]),
...
def estimate_paint_costs():
    ...
    img = cv2.imread(
        '../dataset/train/images/' + input("Введите название файла фотографии модели в формате (photo.jpg): "))

    # Параметры, вводимые пользователем
    front_door_area = float(
        input("Введите физический размер передней двери: "))  # Эталонная площадь передней двери (м²)
    ...
    paint_cost_per_liter = float(
        input("Введите стоимость 1 литра ЛКМ: "))  # Стоимость 1 литра ЛКМ (в условных единицах)
